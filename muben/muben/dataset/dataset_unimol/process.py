...
def conformer_sampling(coordinates: List[Matrix]):
    ...
    Matrix
        Sampled coordinates.
    """
    # Was `assert len(coordinates) == 11` (10 3D + 1 2D fallback, matching
    # the original paper). Relaxed to accept any nonzero count: for frozen
    # feature extraction (one forward pass per molecule, no multi-epoch
    # training where seeing different conformers aids generalization),
    # generating 10 and randomly discarding 9 is equivalent in expectation
    # to generating 1 directly -- this project's billion-scale conformer
    # generation uses --n-conformer 1 for a ~9x speedup and ~2.3x smaller
    # on-disk cache (both measured directly), which this assert would
    # otherwise reject.
